...
import MassMockDialog from './components/developer/MassMockDialog.vue';
import RecordDetailsDialog from './components/developer/RecordDetailsDialog.vue';
...
const activeDeveloperTab = ref<'vault' | 'trash' | 'neurons'>('vault');
...
const viewDialogRecord = ref<any>(null);
const viewDialogVisible = ref(false);
const showRawSynapses = ref(false); // 🧠 HAM VERİ GÖSTERİMİ İÇİN

const openViewDialog = (record: any) => {
  if (record.content.includes('Şifreli Mühür') || record.content.includes('Şifreli Silinen Veri')) return;
  viewDialogRecord.value = record;
  showRawSynapses.value = false; // Reset toggle
  viewDialogVisible.value = true;
};

// 🧠 Mock Veri Üretici Dialogu Açma
...
const sendMassMockData = (text: string) => {
  if (!text.trim()) return;
  const mocks = text.split('\n').filter(line => line.trim().includes(':'));

  if (socket.value && socket.value.readyState === WebSocket.OPEN) {
    mocks.forEach((msg, index) => {
      setTimeout(() => {
        socket.value.send(JSON.stringify({ type: 'SAVE', content: msg.trim().normalize('NFC'), level: 2, owner: 'Sezgin' }));
        logs.value.unshift({ id: Date.now() + index, text: `Mock Veri Gönderildi: ${msg}`, type: 'info' });
      }, index * 200);
    });
    // Verileri ve nöronları tazele
    setTimeout(() => {
      fetchAllRecords();
    }, mocks.length * 200 + 500);

    mockText.value = ''; // Bu satırı koruyalım veya silebiliriz, alt bileşen zaten sıfırlıyor
  ...
  socket.value.onmessage = (event: MessageEvent) => {
    try {
      ...
      if (Array.isArray(rawData)) {
        ...
        const recordIndex = allRecords.value.findIndex(r => r.id === rawData.id);
        if (recordIndex !== -1) {
          allRecords.value[recordIndex] = {
            ...allRecords.value[recordIndex],
            content: rawData.content,
            synapses: rawData.synapses,
            primes: rawData.primes
          };
          logs.value.unshift({ id: Date.now(), text: `Veri çözüldü: ID ${rawData.id}`, type: 'success' });
        }
        
        const trashRecordIndex = trashRecords.value.findIndex(r => r.id === rawData.id);
        if (trashRecordIndex !== -1) {
          trashRecords.value[trashRecordIndex] = {
            ...trashRecords.value[trashRecordIndex],
            content: rawData.content,
            synapses: rawData.synapses,
            primes: rawData.primes
      ...
      } else if (rawData.type === "ALL_NEURONS") {
        neurons.value = rawData.data; // 🧠 Nöron listesini kaydet
        ...
        chatMessages.value.push({ id: Date.now(), role: 'ai', text: rawData.text, sources: rawData.sources });
...
const askAI = () => {
  ...
  if (socket.value && socket.value.readyState === WebSocket.OPEN) {
    socket.value.send(JSON.stringify({ type: 'AI_QUERY', content: content.value.normalize('NFC') }));
  }
  content.value = '';
  scrollToBottom();
};
...
const fetchAllRecords = () => {
  if (socket.value?.readyState === WebSocket.OPEN) {
    ...
    socket.value.send(JSON.stringify({ type: 'GET_ALL_NEURONS' })); // 🧠 Nöronları da tazele
  }
};
...
<template>
  <div class="h-screen bg-gray-950 text-gray-100 font-sans flex overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6">
          <div class="flex items-center justify-between h-16">
            <div class="flex items-center gap-3">
              ...
              <span
                class="text-xl font-black bg-gradient-to-r from-blue-400 via-indigo-400 to-cyan-400 bg-clip-text text-transparent tracking-tight">
                Cortex Core AI
              </span>
            </div>

            <div
              class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-800/80 border border-gray-700/50 text-[11px] font-semibold">
      ...
      <!-- Sub views layout max-w-7xl -->
      <main class="flex-1 w-full mx-auto p-2 overflow-hidden flex flex-col h-full relative">

        <!-- Chat View -->
        <ChatContainer v-show="activeTab === 'chat'" ref="chatRef" v-model="content" :chatMessages="chatMessages"
          :isAiThinking="isAiThinking" :isRecording="isRecording" @ask-ai="askAI" @toggle-recording="toggleRecording"
          @send-data="sendData" />

        <!-- Developer View -->
        <div v-show="activeTab === 'developer'"
          class="grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-y-auto custom-scrollbar h-full pt-1 pb-4">
          <LogsViewer :logs="logs" class="lg:col-span-1" />
          <VaultTable v-model:activeDeveloperTab="activeDeveloperTab" @generate-mock="openMockDialog"
            :allRecords="allRecords" :trashRecords="trashRecords" :neurons="neurons" @refresh-records="fetchAllRecords"
            @refresh-trash="fetchTrashRecords" @empty-trash="emptyTrashBtn" @fetch-payload="fetchPayload"
            @soft-delete="softDeleteRecord" @restore="restoreRecord" @open-view="openViewDialog" class="lg:col-span-2" />
        </div>
    ...
    <Dialog v-model:visible="showBioAuth" modal :showHeader="false" :style="{ width: '400px' }"
      ...
      <div class="bg-gray-900 border border-gray-700 rounded-2xl overflow-hidden">
        ...
        <div class="p-6 flex flex-col items-center text-center gap-4">
          ...
          <p class="text-sm text-gray-400">Hassas Level 3 veriyi saklıyorsunuz. Kripto kilitleme için parmak izinizi
            onaylayın.</p>
    ...
    <!-- Content View Dialog -->
    <RecordDetailsDialog v-model:visible="viewDialogVisible" :record="viewDialogRecord" :neurons="neurons" />

    <!-- 📄 Mass Mock Data Dialog -->
    <MassMockDialog v-model:visible="mockDialogVisible" @submit-mock="sendMassMockData" />
